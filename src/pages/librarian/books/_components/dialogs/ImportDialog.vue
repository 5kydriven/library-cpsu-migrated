<script setup>
	...
	const handleFileChange = (event) => {
		...
		Papa.parse(file.value, {
			...
			complete: (results) => {
				...
				const lowerCaseHeaders = results.meta.fields.map((field) =>
					field.toLowerCase(),
				);
	...
	const uploadFile = async () => {
		...
		files.value.forEach((fileData) => {
			const newDocRef = doc(booksCollection);
			...
			const cleanedData = {
				...
				price: parsePrice(fileData.price),
				totalPrice: parsePrice(fileData.totalprice),
	...
	<Dialog
		...
		<div>
			<div>
				<DataTable
					...
					<Column
						field="stocks"
						header="Stocks"></Column>
					<Column
						field="college"
						header="College"></Column>
					<Column
						field="title"
						header="Title"></Column>
					<Column
						field="author"
						header="Author"></Column>
					<template #footer>
						In total there are {{ files ? files.length : 0 }} Book's
					</template>
				</DataTable>
			</div>
		</div>
		...
		<span
			...
			<div role="status">
				<svg
					aria-hidden="true"
					class="w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 fill-primary-500"
					viewBox="0 0 100 101"
					fill="none"
			...
			<label class="text-gray-50 rounded px-0.5 py-0.5 font-semibold"
				>"Please wait, this may take a moment."</label
			>
		</span>
	</Dialog>
</template>
